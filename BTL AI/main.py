...
img = Image.open("Pics\\HANGMA_GUIcopy.png")
imm = img.resize((mapsize, mapsize))
imm.save("Pics\\resized.png")
...
# import map on graph
graph = window["-GRAPH-"]
image = graph.draw_image(filename="Pics\\resized.png", location=(0, mapsize))
...
def reset():
    ...
    image = graph.draw_image(
        filename="Pics//resized.png", location=(0, mapsize))
...
def draw_way(xuatPhat, dichDen):
    ...
    if buttonChooseDi == 1 and buttonChooseDen == 0:
        ...
        graph.draw_circle(q1, 3, fill_color='blue',
                            line_color='blue', line_width=3)
        linee = "- Đi bộ ra " + list_pho[xuatPhat.thuoc_pho].ten_pho
    ...
    if buttonChooseDi == 1 and buttonChooseDen == 1:
        ...
        graph.draw_circle(q1, 3, fill_color='blue',
                            line_color='blue', line_width=3)
        linee = "- Đi ra " + list_pho[xuatPhat.thuoc_pho].ten_pho
        directionList.append(linee)
        directionLines += linee + "\n"
        window["-Direction-"].update(directionLines)
        window.refresh()
    ...
    # comboDi, comboDen
    if buttonChooseDi == 0 and buttonChooseDen == 0:
        graph.draw_image(filename="Pics\\resized_marker.png",
...
def draw_5_9(X1, X2): # y-coordinate decreasing
    ...
    with open('Toan\\NamChin.txt', 'r', encoding='utf-8') as file:
        data59.append(coor5)
        for line in file:
            line = line.strip()
            if line == '***':
        ...
        case "-ChooseDen-":
            ...
            if event == "-GRAPH-":
                ...
                if in_hang_ma(x1, x2):
                    ...
                    graph.draw_image(filename="Pics\\resized_marker.png", location=(
